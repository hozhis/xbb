...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keyup','input.ui-pg-input', function(e) {
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: grid.width+"px",padding:0+"px"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
		...
		$(grid.bDiv)
			...
			.css({ height: "auto", width: (grid.width)+"px"})
